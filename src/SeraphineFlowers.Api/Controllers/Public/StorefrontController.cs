...
public sealed record RegisterCustomerRequest(string Phone, string Name);
public sealed record CustomerRegistrationResponse(bool Success, bool IsNew, CustomerDto Customer);
public sealed record CustomerLookupRequest(string Phone);
...
public sealed record StorefrontConfigDto(bool RequireCustomerOtp, int WelcomeDiscount, int LoyaltyDiscount, int LoyaltyEvery);
